        ...
        viewModelScope.launch {
            ...
            when(qrCodeResult.type) {
                ...
                QRCodeResultType.TIMEOUT -> {
                    ...
                    addLogUseCase.qrFailLog("QR 인증 실패", qrCodeResult.dataBody ?: result.text, qrCodeResult.message ?: "")
                }
                ...
                is Response.Success -> {
                    when(verifyQRCodeUseCase(qrCodeResult, responseUser.data)) {
                        ...
                        VerifyQRCodeUseCase.VerifyQRCodeResult.ERROR_UNREGISTERED -> {
                            ...
                            addLogUseCase.qrFailLog("QR 인증 실패", result, "${responseUser.data.name} - QR 미등록")
                        }
                        VerifyQRCodeUseCase.VerifyQRCodeResult.ERROR_PASSWORD -> {
                            showToolTipMessage("비밀번호가 일치하지 않습니다.")
                            addLogUseCase.qrFailLog("QR 인증 실패", result, "${responseUser.data.name} - 비밀번호 오류")
                        }
                        VerifyQRCodeUseCase.VerifyQRCodeResult.ERROR_KB_PASS -> {
                            showToolTipMessage("고유키가 일치하지 않습니다.")
                            addLogUseCase.qrFailLog("QR 인증 실패", result, "${responseUser.data.name} - KBPASS 오류")
                        }
                        VerifyQRCodeUseCase.VerifyQRCodeResult.ERROR_DEVICE_ID -> {
                            showToolTipMessage("단말기 고유값이 일치하지 않습니다.\n다시 등록해 주세요.")
                            addLogUseCase.qrFailLog("QR 인증 실패", result, "${responseUser.data.name} - DEVICE ID 오류")
                        }
                        else -> {
                    ...
                    addLogUseCase.qrFailLog("QR 인증 실패", result, "${id} - 미등록 사용자")
                }
            }
        }
    }
